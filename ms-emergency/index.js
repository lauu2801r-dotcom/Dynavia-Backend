...
app.post('/emergency/activate', async (req, res) => {
  const { ambulance_id, level, severity_level, punto_a_lat, punto_a_lng, punto_b_lat, punto_b_lng } = req.body;
  const finalLevel = Number(severity_level || level);

  if (!ambulance_id || !finalLevel) return res.status(400).json({ error: 'Faltan campos requeridos' });
  if (![1, 2, 3].includes(finalLevel)) return res.status(400).json({ error: 'Nivel debe ser 1, 2 o 3' });
  ...
  try {
    const notifRes = await fetch(`${NOTIFICATIONS_URL}/notify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ event_id, vehicle_id: ambulance_id, level: finalLevel, position: 'front' })
    });
    const notifData = await notifRes.json();
    vehicles_notified = notifData.status === 'sent' ? 1 : 0;
  ...
  try {
    ...
    res.status(201).json({ message: 'Modo emergencia activado', event_id, activated_at, level: finalLevel, semaphores_activated, vehicles_notified });
  } catch (err) {
...
app.post('/emergency/deactivate', async (req, res) => {
  ...
  try {
    const found = await pool.query(`SELECT * FROM emergency_events WHERE id = $1`, [event_id]);
    if (found.rows.length === 0) return res.status(404).json({ error: 'Evento no encontrado en BD' });

    const event = found.rows[0];
    const deactivated_at = new Date();
    const total_duration_seconds = Math.floor((deactivated_at - new Date(event.activated_at)) / 1000);
    ...
    await pool.query(
      `UPDATE emergency_events SET status='completed', deactivated_at=$1, total_duration_seconds=$2, distance_km=$3 WHERE id=$4`,
    ...
    } catch (e) { console.warn('⚠️ ms-traffic restore no respondió:', e.message); }
    ...
    } catch (e) { console.warn('⚠️ ms-metrics no respondió:', e.message); }

    console.log(`✅ Emergencia cerrada: ${event_id} - ${total_duration_seconds}s - ${distance_km}km`);
    res.json({ message: 'Emergencia desactivada', event_id, activated_at: event.activated_at, deactivated_at, total_duration_seconds, duration_minutes: (total_duration_seconds/60).toFixed(2), distance_km, semaphores_activated: event.semaphores_activated || 0, vehicles_notified: event.vehicles_notified || 0 });
...
app.get('/emergency/:event_id', async (req, res) => {
  try {
    const result = await pool.query(`SELECT * FROM emergency_events WHERE id=$1`, [req.params.event_id]);
...
app.get('/emergency', async (req, res) => {
  try {
    const result = await pool.query(`SELECT * FROM emergency_events ORDER BY activated_at DESC LIMIT 20`);
    res.json({ events: result.rows });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.get('/health', (req, res) => res.json({ status: 'ok', service: 'ms-emergency', db: 'neon' }));

const PORT = process.env.PORT || 3001;
app.listen(PORT, () => console.log(`🚀 ms-emergency corriendo en puerto ${PORT}`));
